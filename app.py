...
def html_code_table(prod_df,table_name,file_name,side):
    ...
    for i in range(len(prod_df.index)):
        row = '<tr><td>' + str(prod_df['product_name'][i]) + '</td><td>' + str(prod_df['price'][i]) + '</td></tr>'
...
def most_popular_table():
    most_popular_prods = prod_ranking_model.sort_values('Popularity_Rank',ascending=True)[['product_name','price']].head(10).reset_index(drop=True)
...
def top_sell_table():
    top_sell_prods = prod_ranking_model.sort_values('Top_Sell_Rank',ascending=True)[['product_name','price']].head(10).reset_index(drop=True)
...
def cust_most_popular_table(cust_name):
    ...
    cust_most_popular_prods = cust_most_popular_prods.sort_values('Popularity_Rank',ascending=True)[['product_name','price']].head(10).reset_index(drop=True)
...
def cust_top_sell_table(cust_name):
    ...
    cust_top_sell_prods = cust_top_sell_prods.sort_values('Top_Sell_Rank',ascending=True)[['product_name','price']].head(10).reset_index(drop=True)
...
def recommend_prod_cust(cust_name):
    ...
    for i in range(len(similar_custs_corr)):
        if similar_custs_corr.index[i] != cust_name:
            ...
            cust_top_sell_prods = cust_top_sell_prods[['product_name','Qty','price']].reset_index(drop=True)
    ...
    prod_by_similar_custs = prod_by_similar_custs.groupby('product_name').agg({'Qty_Corr':'sum','price':'max'})
    ...
    prod_recommend_to_cust = prod_recommend_to_cust.sort_values('Qty_Corr',ascending=False)[['product_name','price']].head(10).reset_index(drop=True)
...
def similar_prods(prod_name):
    ...
    similar_prods = pd.merge(similar_prods_corr,prod_ranking_model[['product_name','price']],how='left',on='product_name')
    
    prod_price = similar_prods[similar_prods['product_name'] == prod_name]['price'].values[0]
